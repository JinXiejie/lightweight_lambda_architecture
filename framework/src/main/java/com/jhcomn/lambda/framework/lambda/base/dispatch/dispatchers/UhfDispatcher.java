...
import com.jhcomn.lambda.mllib.uhf.UHFController;
import com.jhcomn.lambda.mllib.uw1000.UW1000Feature;
import com.jhcomn.lambda.mllib.uhf.UhfResult;
...
import com.jhcomn.lambda.packages.result.UHF.UhfResultModel;
import org.apache.hadoop.fs.Path;

import java.util.ArrayList;
import java.util.Date;
import java.util.List;
public class UhfDispatcher extends BaseDispatcher {
    public UhfDispatcher() {
        imlController = new UHFController(spark, fileSystem);
    }
    ...
    protected void fullCompute(String date, BatchUpdatePkg ipkg) {
        System.out.println("batch layer start UHF full computation, date is " + date);
...
//                        System.out.println("model：" + model.toString());
    ...
    protected void analyze(String date, HdfsFetchTTMetaPkg metaPkg) {
        ...
        List<UhfProcessResultModel> results = new ArrayList<>();
        UhfResultModel out = new UhfResultModel(metaPkg.getId(),
                metaPkg.getUid(), metaPkg.getType(), results);

        metaPkg.getFiles().stream().filter(pkg -> pkg != null).forEach(pkg -> {
            //用于存储处理结果的model
            UhfProcessResultModel processResult = new UhfProcessResultModel();
            ...
            System.out.println("开始调用模型分析UHF局放数据:" + lists.size());
            ...
            if (lists != null) {
                for (IPackage data : lists) {
                    ...
                    if (result != null) {
                        processResult.setTag(((UhfResult)result).getTags());
                    }
        ...
        ProducerPackage producerPackage = new ProducerPackage(ConstantDatas.UHF_RESULT, out.getId(), json);
        ...
        System.out.println("UHF result save or update in mysql and send to kafka broker end...");
    }
