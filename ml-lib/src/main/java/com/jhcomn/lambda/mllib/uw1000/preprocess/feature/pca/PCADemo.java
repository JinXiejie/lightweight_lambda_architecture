 ...
 * principal components. The data file should be in the same directory as the PCADemo.class file.
...
public class PCADemo {
	...
	/**
	 * Performs PCADemo on a set of data and prints the resulting transformed data set.
...
//		double[][] results = PCA.principalComponentAnalysisTraining(data, numComps);
//		System.out.println(numComps + " principal components:");
//		Matrix.print(results);
//		saveResults(results, args[0]);
		
//		double[][] scores = PCA.PCANIPALS(data, numComps);
...
//		saveResults(scores, args[0]);
	...
	/**
	 * Saves the results of PCADemo to a file. The filename has "_processed" appended to it before
	 * the extension.
	 * @param results	double[][] of PCADemo results
