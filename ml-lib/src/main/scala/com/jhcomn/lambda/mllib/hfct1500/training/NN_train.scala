package com.jhcomn.lambda.mllib.hfct1500.training

import com.jhcomn.lambda.mllib.hfct1500.classify.HFCT_identify
...
import com.jhcomn.lambda.mllib.hfct1500.training.NN.NeuralNet
...
object NN_train {
  def main(args: Array[String]) {
    ...
    //    //2 读取样本数据,
    Logger.getRootLogger.setLevel(Level.WARN)
    val data_path = "C:\\learnscala\\testtwo\\SparkMLlibDeepLearn-master\\SparkMLlibDeepLearn-master\\src\\Data\\myFile.txt"
    val examples = sc.textFile(data_path).cache()
    val train_d1 = examples.map { line =>
      val f1 = line.split("\t")
      val f = f1.map(f => f.toDouble)
      //val id = f(0)
      //val y = Array(f(0),f(1),f(2))
      val y=f.slice(0,3)   //可以写0到类别数n
    val x = f.slice(3, f.length)
    ...
    val opts = Array(10.0, 1000.0, 0.0) //第二个元素是迭代次数
    //3 设置训练参数，建立模型
    val NNmodel = new NeuralNet().
      setSize(Array(7200,10, 3)).
      setLayer(3).
      setActivation_function("sigm").
      setLearningRate(2.0).
      setScaling_learningRate(1.0).
    ...
    //val json = JSON.toJSONString(NNmodel)
    //println("json:" + json)
    //    sc.textFile("/nn.model")
    ...
    val printf1 = NNforecast.map(f => (f.label.data(0),f.label.data(1),f.label.data(2),f.predict_label.data(0),f.predict_label.data(1),f.predict_label.data(2))).take(200)
    ...
    val test_train=train_d.map{f=>
      val label=f._1
      val feature=f._2
      feature
    }
    val test_train1=test_train.first()
    val ouput=HFCT_identify.Hfct_identify_output(test_train1,NNmodel.weights)
    println(ouput)



    /*
        val printf1 = NNforecast.map(f => (f.label.data(1), f.predict_label.data(2))).take(200)
        println("预测结果——实际值：预测值：误差")
        for (i <- 0 until printf1.length)
          println(printf1(i)._1 + "\t" + printf1(i)._2 + "\t" + (printf1(i)._2 - printf1(i)._1))
    */
    ...
    */
    ...
    /*
    val out = new PrintWriter("w12.txt")
    for (i <- 0 to tmpw0.rows - 1) {
      for (j <- 0 to tmpw0.cols - 1) {
        out.print(tmpw0(i, j) + "\b")
      }
      out.print("\n")
    }
